...
const SESSION_SECRET        = process.env.SESSION_SECRET || 'balius-dashboard-secret';
...
async function startDashboard(client) {
  ...
  app.get(['/auth/callback', '/callback'], async (req, res) => {
  ...
  server.on('error', (err) => {
    if (err && err.code === 'EADDRINUSE') {
      console.warn(`[Dashboard] port ${PORT} already in use — skipping dashboard startup`);
    } else {
