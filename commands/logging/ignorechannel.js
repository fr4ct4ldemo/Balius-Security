...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('action').setDescription('Action').setRequired(true).addChoices({ name: 'Add (ignore)', value: 'add' }, { name: 'Remove (unignore)', value: 'remove' }, { name: 'List', value: 'list' }))
  ...
  async execute(interaction, client) {
    try {
      ...
      if (action === 'add') {
        if (db.isChannelIgnored(guildId, channel.id)) return interaction.editReply({ embeds: [errorEmbed('❌ Already Ignored', `<#${channel.id}> is already ignored.`)] });
      ...
      if (action === 'remove') {
        if (!db.isChannelIgnored(guildId, channel.id)) return interaction.editReply({ embeds: [errorEmbed('❌ Not Ignored', `<#${channel.id}> is not in the ignore list.`)] });
