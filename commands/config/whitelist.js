...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('type').setDescription('Type of entry').setRequired(true).addChoices({ name: 'User', value: 'user' }, { name: 'Role', value: 'role' }, { name: 'Channel', value: 'channel' }, { name: 'Link (domain)', value: 'link' }))
  ...
  async execute(interaction, client) {
    try {
      ...
      if (action === 'list') {
        ...
        const display = entries.length ? entries.map(e => type === 'user' ? `<@${e}>` : type === 'role' ? `<@&${e}>` : type === 'channel' ? `<#${e}>` : `\`${e}\``).join('\n') : `No ${label} whitelist entries.`;
      ...
      if (action === 'add') {
        if (db.isWhitelistedEntry(guildId, type, value)) return interaction.editReply({ embeds: [errorEmbed('❌ Already Whitelisted', `\`${value}\` is already in the ${type} whitelist.`)] });
      ...
      if (action === 'remove') {
        if (!db.isWhitelistedEntry(guildId, type, value)) return interaction.editReply({ embeds: [errorEmbed('❌ Not Found', `\`${value}\` is not in the ${type} whitelist.`)] });
