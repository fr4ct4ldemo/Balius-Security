...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('type').setDescription('Type of entry').setRequired(true).addChoices({ name: 'User (auto-ban on join)', value: 'user' }, { name: 'Link (domain)', value: 'link' }, { name: 'Word', value: 'word' }))
  ...
  async execute(interaction, client) {
    try {
      ...
      if (action === 'list') {
        ...
        const display = entries.length ? entries.map(e => type === 'user' ? `<@${e}>` : `\`${e}\``).join('\n') : `No ${label} blacklist entries.`;
      ...
      if (action === 'add') {
        if (db.isBlacklisted(guildId, type, value)) return interaction.editReply({ embeds: [errorEmbed('❌ Already Blacklisted', `\`${value}\` is already in the ${type} blacklist.`)] });
      ...
      if (action === 'remove') {
        if (!db.isBlacklisted(guildId, type, value)) return interaction.editReply({ embeds: [errorEmbed('❌ Not Found', `\`${value}\` is not in the ${type} blacklist.`)] });
