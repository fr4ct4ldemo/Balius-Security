...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('action').setDescription('Action').setRequired(true).addChoices({ name: 'Enable', value: 'enable' }, { name: 'Disable', value: 'disable' }, { name: 'Setup (send verify panel)', value: 'setup' }, { name: 'Status', value: 'status' }))
  ...
  async execute(interaction, client) {
    try {
      ...
      if (action === 'status') {
        ...
        return interaction.editReply({ embeds: [successEmbed('🔐 Verification Status', `**Enabled:** ${current.enabled ? 'Yes' : 'No'}\n**Channel:** ${ch}\n**Role:** ${rl}`)] });
      }
      if (action === 'enable') {
      ...
      if (action === 'setup') {
        ...
        const panelEmbed = successEmbed('✅ Verify Yourself', `Welcome to **${interaction.guild.name}**!\nClick the button below to verify and gain access.`);
        const button = new ButtonBuilder().setCustomId('verify_button').setLabel('Verify').setStyle(ButtonStyle.Success).setEmoji('✅');
        await targetChannel.send({ embeds: [panelEmbed], components: [new ActionRowBuilder().addComponents(button)] });
